...
def parse_arguments():
    ...
    parser.add_argument("--csv", "-f", type=str, required=True,
                        help="Path to the CSV file with 'Smiles' column.")
    parser.add_argument("--algo", "-a", type=str, choices=["kmeans", "maxmin"], default="kmeans",
...
def main():
    ...
    metrics_info = "\n".join([f"{metric}: {score:.4f}" for metric, score in metric_scores.items()])
