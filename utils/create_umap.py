...
def generate_umap(csv_path, labels, n_bits=2048, n_neighbors=15, min_dist=0.1, random_state=42,
    ...
    for i, label in enumerate(unique_labels):
        ...
        plt.scatter(umap_results[indices, 0], umap_results[indices, 1],
                    color=colors[i], label=f'Cluster {label}', alpha=0.7, s=20)

    plt.title("UMAP visualization of clustered molecules")
    plt.xlabel("UMAP 1")
    plt.ylabel("UMAP 2")
